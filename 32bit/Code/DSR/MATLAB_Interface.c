...
Engine *ep;
int status;
mxArray *Xc = NULL, *Yc = NULL, *Pwr = NULL, *max_energy = NULL;
double **x, **y, **p;
mxArray *out;
double *result[3];
int dim = 0;
...
void fn_netsim_matlab_run(int s_count, int n_cls)
{
	...
	sprintf(buf, "[A,B,C]=som_optimization([Xc,Yc],%d,%d,Pwr,max_energy)", s_count, n_cls);
	status = engEvalString(ep, buf);
	...
	fn_netsim_som_form_clusters(cluster_id, cluster_size);//cluster id's and size of each cluster are passed as parameters
